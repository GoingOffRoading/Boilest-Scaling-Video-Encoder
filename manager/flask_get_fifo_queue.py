...
def get_fifo_queue_logic(db_disabled, worker):
    import logging
    logging.info("=" * 80)
    logging.info("[REQUEST] GET /api/queue/fifo")
    logging.info("=" * 80)
    ...
    try:
        ...
        query = """
            ...
            LIMIT 1
        """
        logging.debug("[QUERY] Executing FIFO query by queue ROWID...")
        cur.execute(query)

        row = cur.fetchone()
        logging.debug("[QUERY] Query executed successfully")
        ...
        if row:
            result = dict(row)
            logging.info(f"[RESULT] Queued for encoding (FIFO): {result['input_file_name']}")
            logging.info("=" * 80)
            return {
                'success': True,
                'data': result
