...
__all__ = ["get_smallest_queue_logic"]


def get_smallest_queue_logic(db_disabled, worker):
    import logging
    logging.info("="*80)
    logging.info("[REQUEST] GET /api/v2/queue/smallest")
    logging.info("="*80)
    ...
    try:
        ...
        # Query the encode table sorted by before_file_size ascending, limit 1
        query = """
            SELECT 
                file_guid,
                directory_path,
            ...
            FROM queue q
            WHERE status = 'queued'
            ORDER BY before_file_size ASC
            LIMIT 1
        """
        logging.debug("[QUERY] Executing query to fetch smallest queue by before_file_size...")
        cur.execute(query)
