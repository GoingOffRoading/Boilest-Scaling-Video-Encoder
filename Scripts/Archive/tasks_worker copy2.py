...
def fencoder(ffmpeg_inputs):
    ...
    if os.path.exists(ffmpeg_inputs['file_path']):
        logging.debug (ffmpeg_inputs['file_path'] + ' Exists')
        ready_command = 'ready_to_check_hash'


    # Lets check to see if the current file is the same file as when it was scanned
    if ready_command == 'ready_to_check_hash':
        logging.debug ('Checking to see if the file matches the file that was scanned')
        if get_file_size_bytes(ffmpeg_inputs['file_path']) == ffmpeg_inputs['file_hash']:
            logging.debug ('file matches')
            ready_command = 'ready_to_validate_source'
        else:
            logging.debug ('file does not match, skipping')
